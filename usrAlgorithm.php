...
if ($_SESSION['MM_UserGroup'] == "user") {
	...
	$query_Recordset_Alg = sprintf("SELECT * FROM solution WHERE usr_id = %s ORDER BY createDate DESC", GetSQLValueString($colname_Recordset_Alg, "int", $conn));
}
else {
	$query_Recordset_Alg = sprintf("SELECT * FROM solution ORDER BY createDate DESC");
}
  ...
  <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation"> 
    ...
    <div class="collapse navbar-collapse navbar-ex1-collapse">
      <ul class="nav navbar-nav side-nav">
        ...
        <li class="active"><a href="#"><i class="fa fa-font"></i> My Algorithm</a></li>
  ...
  <div id="page-wrapper">
    <div class="row">
      <div class="col-lg-12">
        ...
        <ol class="breadcrumb">
          <li><a href="usrAdmin.php"><i class="fa fa-dashboard"></i> Profile Page</a></li>
          <li class="active"><i class="fa fa-table"></i> My Algorithm</li>
        </ol>
    ...
    <div class="row" id="div_addInfo">
      <div class="col-lg-12">
        <h2 id="head2">View Uploaded Algorithm</h2>
        <div class="panel panel-primary">
          <div class="panel-heading">
            <h3 class="panel-title"><i class="fa fa-bar-chart-o"></i> Algorithm Interface</h3>
          </div>
          <div class="panel-body">
            <div class="flot-chart">
              <div class="flot-chart-content" id="flot-chart-line">
                <table id="exist" width="300" border="0" align="center" class="table table-bordered table-hover tablesorter">
                  ...
                  <tr>
                    ...
                    <td><?php 
						  ...
						  echo '<a href="showProblem.php?proId='.$colname_Recordset_Problem.'">'.$row_Recordset_Problem['name'].'</a>'; 
						  ?></td>
                    <td><?php 
                    ...
                    <td><?php echo $row_Recordset_Alg['runTimes']; ?></td>
					  <?php 
					  if($_SESSION['MM_UserGroup'] == "admin") { 
						  $query_Recordset_User = sprintf("SELECT name FROM user WHERE id = %s", GetSQLValueString($row_Recordset_Alg['usr_id'], "int", $conn));
					  ...
					  ?> 
                    <td><button class="btn btn-default" id="description<?php echo $row_Recordset_Alg['id']; ?>" value="<?php echo $row_Recordset_Alg['id']; ?>">View</button></td>
                    <td><a href="<?php echo "file\Solution\Sol-".$row_Recordset_Alg['id']."\download\\".$row_Recordset_Alg['downloadfile'] ?>" download="<?php echo $row_Recordset_Alg['downloadfile']?>">
                      <button class="btn btn-default">Download</button>
                      </a></td>
					<td><button class="btn btn-default" id="del<?php echo $row_Recordset_Alg['id'] ?>" value=<?php echo $row_Recordset_Alg['id'] ?>>Delete</button></td>
                  </tr>
				...
				<div>
					<button class="btn btn-default" id="tips" value="<?php if($_SESSION['MM_UserGroup']=='user') echo "userAlgorithm";else echo "adminAlgorithm"; ?>">Tips</button>
				</div>
              </div>
            </div>
          </div>
...
$("button[id^='description']").on('click', function(){
    ...
    layer.open({
    	...
    	shade: 0.8,
    	area: ['480px', '550px'],
    	content: 'readtxt.php?id=' + id //iframe的url
	});
});
</script>
...
$("button[id^='del']").on('click', function(){
   ...
   var id = $(this).val();
	...
	window.location.href='del.php?algId=' + id;
	});
});
</script>
